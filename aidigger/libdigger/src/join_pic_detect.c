...
// compare to sort detection** by best_class probability, used for qsrot DECENCDING
...
void report_detection(image im, detection *dets, int num, float thresh, char **names, int classes, int ext_output, unsigned char* result_512bits, const char* filename){
    ...
    logm(SL4C_DEBUG,"converting finished\n");
}
    ...
    for(i = 0; i < num; ++i){
        ...
        for(j = 0; j < classes; ++j){
            if (dets[i].prob[j] > thresh){
                ...
                fprintf(f, "%f\t", names[j], dets[i].prob[j]);
            }
        }
    }
    fprintf(f,"\n");
...
void test_detector_cpu_folder(char **names, char *cfgfile, char *weightfile, char *foldername, float thresh, int quantized, int dont_show){
    image **alphabet = NULL;
...
#ifdef GPU
            const char * detect_log = "detect_log_gpu.txt";
#else
            const char * detect_log = "detect_log_cpu.txt";
#endif
    ...
    {
            //see if .. or . is in the file name
            DIR *possible_dir = opendir(dp->d_name);
            if (possible_dir)
                continue;

            char* absFilename = buff;
            strcpy(absFilename,foldername);               
            strcat(absFilename,dp->d_name);   


            image im = load_image(absFilename, 0, 0, 3);            // image.c
            ...
            // }
            save_detect_info(detect_log, dets, nboxes, thresh, names, l.classes, absFilename);
    ...
    int frame_skip, char *prefix, int quantized, char *out_filename, int dont_show)
{
    ...
    while (1) {
        ...
        if (!prefix) {
            ...
            char buff[256];
            logm(SL4C_DEBUG,buff, "%s_%08d", prefix, count);
...
void run_test_detector(char* filename,char* obj_names, int dont_show, float thresh, unsigned char* result_512bits, void* net_ptr){
    logm(SL4C_DEBUG,"loading classification names\n");
    int clear = 0;                // find_arg(argc, argv, "-clear");
    int quantized = 0;
    // load object names
    char **names = calloc(10000, sizeof(char *));
