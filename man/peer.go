...
func (p *peer) broadcast() {
	for {
		...
		case txs := <-p.queuedTxs:
			log.Info("man/peer", "broadcast,addr", p2p.ServerP2p.ConvertIdToAddress(p.ID()).String(), "node id", p.ID().String()) //YY
...
func (p *peer) SendTransactions(txser []types.SelfTransaction) error {
	...
	for _, txer := range txser {
		...
		log.Info("man/peer.go", "tcp SendTransactions,txHash", txer.Hash().String()) //YY
	}
	if len(tmptxs) <= 0 {
...
func SendUdpTransactions(txser []types.SelfTransaction) error {
	...
	for _, txer := range txser {
		...
		log.Info("man/peer.go", "udp SendUdpTransactions,txHash", txer.Hash().String()) //YY
	}
...
func (p *peer) SendBlockHeaders(headers []*types.Header) error {
	...
	if len(headers) > 1 {
		for i, item := range headers {
			log.Info("test log", "index", i, "number", item.Number, "version", string(item.Version))
		}
	}
