...
import (
	...
	"github.com/MatrixAINetwork/go-matrix/man/tracers"
...
func (api *PrivateDebugAPI) TraceChain(ctx context.Context, start, end rpc.BlockNumber, config *TraceConfig) (*rpc.Subscription, error) {
	...
	case rpc.PendingBlockNumber:
		from = api.man.miner.PendingBlock()
	case rpc.LatestBlockNumber:
		from = api.man.blockchain.CurrentBlock()
	default:
		from = api.man.blockchain.GetBlockByNumber(uint64(start))
	}
	switch end {
	case rpc.PendingBlockNumber:
		to = api.man.miner.PendingBlock()
	case rpc.LatestBlockNumber:
		to = api.man.blockchain.CurrentBlock()
	default:
		to = api.man.blockchain.GetBlockByNumber(uint64(end))
	}
...
func (api *PrivateDebugAPI) traceChain(ctx context.Context, start, end *types.Block, config *TraceConfig) (*rpc.Subscription, error) {
	...
	database := state.NewDatabase(api.man.ChainDb())

	if number := start.NumberU64(); number > 0 {
		start = api.man.blockchain.GetBlock(start.ParentHash(), start.NumberU64()-1)
		if start == nil {
	...
	go func() {
		...
		for number = start.NumberU64() + 1; number <= end.NumberU64(); number++ {
			...
			block := api.man.blockchain.GetBlockByNumber(number)
			if block == nil {
...
func (api *PrivateDebugAPI) TraceBlockByNumber(ctx context.Context, number rpc.BlockNumber, config *TraceConfig) ([]*txTraceResult, error) {
	...
	switch number {
	case rpc.PendingBlockNumber:
		block = api.man.miner.PendingBlock()
	case rpc.LatestBlockNumber:
		block = api.man.blockchain.CurrentBlock()
	default:
		block = api.man.blockchain.GetBlockByNumber(uint64(number))
	}
...
func (api *PrivateDebugAPI) TraceBlockByHash(ctx context.Context, hash common.Hash, config *TraceConfig) ([]*txTraceResult, error) {
	block := api.man.blockchain.GetBlockByHash(hash)
	if block == nil {
...
func (api *PrivateDebugAPI) traceBlock(ctx context.Context, block *types.Block, config *TraceConfig) ([]*txTraceResult, error) {
	...
	if item, ok := api.man.engine[string(block.Version())]; ok {
		if err := item.VerifyHeader(api.man.blockchain, block.Header(), true); err != nil {
			return nil, err
		}
	} else {
	...
	if err := api.man.engine[string(block.Version())].VerifyHeader(api.man.blockchain, block.Header(), true); err != nil {
		return nil, err
	}
...
func (api *PrivateDebugAPI) computeStateDB(block *types.Block, reexec uint64) (*state.StateDBManage, error) {
	...
	for i := uint64(0); i < reexec; i++ {
		block = api.man.blockchain.GetBlock(block.ParentHash(), block.NumberU64()-1)
		if block == nil {
			break
		}
...
func (api *PrivateDebugAPI) TraceTransaction(ctx context.Context, hash common.Hash, config *TraceConfig) (interface{}, error) {
	...
	tx, blockHash, _, index := rawdb.ReadTransaction(api.man.ChainDb(), hash)
	if tx == nil {
...
// be tracer dependent.
func (api *PrivateDebugAPI) traceTx(ctx context.Context, message txinterface.Message, vmctx vm.Context, statedb *state.StateDBManage, config *TraceConfig) (interface{}, error) {
	...
	case *vm.StructLogger:
		return &manapi.ExecutionResult{
			Gas:         gas,
			Failed:      failed,
			ReturnValue: fmt.Sprintf("%x", ret),
			StructLogs:  manapi.FormatLogs(tracer.StructLogs()),
		}, nil
...
func (api *PrivateDebugAPI) computeTxEnv(blockHash common.Hash, txIndex int, reexec uint64) (txinterface.Message, vm.Context, *state.StateDBManage, error) {
	// Create the parent state database
	block := api.man.blockchain.GetBlockByHash(blockHash)
	if block == nil {
