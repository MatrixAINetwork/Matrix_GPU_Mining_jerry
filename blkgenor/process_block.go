...
func (p *Process) verifyOneResult(rawHeader *types.Header, result *mc.HD_MiningRspMsg) error {
	...
	log.Info("test log", "verify header", header.Hash(), "number", header.Number)
...
func (p *Process) copyHeader(header *types.Header, minerResult *mc.HD_MiningRspMsg) *types.Header {
	...
	if manversion.VersionCmp(string(header.Version), manversion.VersionAIMine) >= 0 {
		newHeader.AIHash = minerResult.AIHash
	}
	return newHeader
}
