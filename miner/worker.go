...
func newWorker(config *params.ChainConfig, bc ChainReader, mux *event.TypeMux, hd *msgsend.HD) (*worker, error) {
	worker := &worker{
		...
		registerAgentCh:      make(chan Agent, 5),
		unRegisterAgentCh:    make(chan Agent, 5),
...
func (self *worker) sendMineResultFunc(data interface{}, times uint32) {
	...
	if resultData.isBroadcastReq {
		...
		if manversion.VersionCmp(string(resultData.header.Version), manversion.VersionAIMine) >= 0 {
			rsp.AIHash = resultData.header.AIHash
		}
